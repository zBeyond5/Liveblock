...
(function() {
    ...
    // Atalho pra reabrir o painel mesmo depois de fechar/minimizar
    ...
    // Diagnóstico: se a página for descartada/recarregada no meio do boot
    // (ex: reação de anti-adblock), isso fica registrado no console mesmo
    // que o painel nunca chegue a aparecer.
  ...
  try {
    ...
    // ─── MANIFESTO (com cache curto + timeout + retry)
    ...
    let syncState = 'loading'; // 'loading' | 'synced' | 'error'
    let lastSyncAt = null;
    ...
    function buildUI() {
        ...
        // Ícone-assinatura: chave de latão (remete à recepção/concierge do hotel)
        const KEY_SVG = `
        ...
        // ─── Pílula minimizada (sempre acessível, mesmo se algo falhar depois)
        ...
        // Drag (arrasta pelo header, ignora cliques em botões)
        ...
        // Atalho de teclado — alterna painel/pílula, funciona assim que buildUI roda
    ...
    // ─── Busca (ou re-busca) o manifesto e atualiza a UI já existente
    ...
    async function boot() {
        // 1) UI (e atalho) sobem primeiro, sem depender de rede.
        await waitForBody();
        buildUI();

        // 2) Só então busca o manifesto. Se isso falhar ou a página recarregar
        //    logo em seguida, o painel e o atalho já existem e continuam usáveis.
