...
(function() {
    ...
    // ═══ MODULE LOADER ═══
    // Baixa via GM_xmlhttpRequest (bypassa CSP/MIME) com fallback pra fetch.
    // Injeta o texto como <script> inline — mesmo padrão do phone/shell.js.
    function loadScript(src, timeoutMs) {
        ...
        return new Promise((resolve) => {
            ...
            const t = setTimeout(() => { console.warn('[Sangzap] timeout', src); finish(false); }, timeoutMs);
            ...
            if (typeof GM_xmlhttpRequest === 'function') {
                try {
                    GM_xmlhttpRequest({
                        ...
                        timeout: timeoutMs,
                        onload: (r) => {
                            if (r.status >= 200 && r.status < 300 && r.responseText) inject(r.responseText);
                            else finish(false);
                        },
            ...
            fetch(src, { cache: 'no-store' })
                ...
                .then(inject)
                .catch(e => { console.warn('[Sangzap] fetch:', e.message); finish(false); });
        });
    }
    ...
    function renderDiagnostic(body) {
        ...
        body.querySelector('#szDiagRetry')?.addEventListener('click', () => {
            ...
            try { delete window._sangzapCtx; } catch(_) {}
            S = null;
    ...
    function openNewChatPicker() {
        ...
        modal.querySelectorAll('.sz-pick').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                } catch(e) { console.warn('[Sangzap] new chat:', e); }
